"""
External module to hold the core components used for (random) video
frame extraction, as used by the main script for this project.
...
def init_parser():
    ...
    # TODO(?): (-t): optional param to invoke project build tests, unit tests, etc.
...
def capture_frames_driver(args) -> bool:
    ...
    if args.n < MIN_SAMPLES_PER_RUN or args.n > MAX_SAMPLES_PER_RUN:
        print(
            "[frame_capture_components] error: "
            "argument 'n' must be between 1 and 100 (inclusive)"
        )
        return False
